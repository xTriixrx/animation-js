...
*
* The Tangent component receives prop values from the Wave component and keeps
* track of time through the Date.now() function. The Tangent component graphs
* the sin() function using <circle> HTML elements.
*/
class Tangent extends PureComponent
{
  /**
  ...
  */
  ...
  /**
  * Renders the current iteration of the tangent graph.
  */
  render()
  {
  ...
  /**
  * Calculates the radius, cx, and cy for each circle in the tangent graph for the
  ...
  */
  circleBuilder = (dataPoints) =>
  {
    ...
    for (var i = 1; i <= dataPoints; i++)
    {
      ...
      var cy = (Math.tan((startPoint + (i * this.state.freqMultiplier)) /
