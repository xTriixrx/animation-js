...
*
* The Sine component receives prop values from the Wave component and keeps
* track of time through the Date.now() function. The Sine component graphs
...
class Sine extends PureComponent
{
  ...
  */
  ...
  /**
  * Renders the current iteration of the sine graph.
  */
  render = () =>
  {
  ...
  /**
  * Calculates the radius, cx, and cy for each circle in the sine graph for the
  ...
  */
  circleBuilder = (dataPoints) =>
  {
    ...
    for (var i = 1; i <= dataPoints; i++)
    {
      ...
      var cy = (Math.sin((startPoint + (i * this.state.freqMultiplier)) /
