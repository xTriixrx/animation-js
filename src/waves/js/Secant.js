...
* Last Updated: 1/9/21
*
* The Secant component receives prop values from the Wave component and keeps
* track of time through the Date.now() function. The Secant component graphs
* the sec() function using <circle> HTML elements.
*/
class Secant extends PureComponent 
{
  /**
  ...
  /**
  * Renders the current iteration of the secant graph.
  */
  render()
  {
  ...
  /**
    * Calculates the radius, cx, and cy for each circle in the secant graph for the
  ...
  circleBuilder = (dataPoints) =>
  {
    ...
    for (var i = 1; i <= dataPoints; i++)
    {
      ...
      var cy = (sec((startPoint + (i * this.state.freqMultiplier)) /
